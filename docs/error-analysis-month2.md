...
`cut/ex` = `add/ex` = 0.0 across every event: LLM #1 (extractor) produced zero explicit ops on all 20 examples in this run, so `localize()` never had participants to build candidates from, and the judge (LLM #2) was never invoked (`llm_calls` = 20, all from the extractor). This isolates the current bottleneck to LLM #1's zero-shot/few-shot extraction on this 2B model, not to the localization/judge hardening done in Task 13 — see `gold_explicit + llm judge` in the README for the run where the judge *does* receive real candidates (n=20, 19 scored, idx=15 context-overflow correctly handled as a per-example error record instead of aborting the run — the pipeline/error-handling path runs end-to-end). That run is NOT evidence the judge works well, though: on the 19 scored examples it emits INVALIDATE for 1329/1332 (99.8%) cut candidates (`judged/cands = 1329/1332`) and ADD for only 4/232 add candidates — a degenerate "always-INVALIDATE" policy with no discriminative signal. Because structural localization has coverage 1.0 (the cut-candidate set already contains every gold deletion), `del_acc = 1.0` under that policy is an arithmetic guarantee, not a sign the judge judged correctly — it also wrongly deletes most still-valid edges (recall vs. `gold_explicit + none` drops 0.926 → 0.894, ~671 valid edges removed). So this run confirms the pipeline plumbing (candidates → judge → apply → error handling) is wired correctly, not that the 2B judge is any good.
